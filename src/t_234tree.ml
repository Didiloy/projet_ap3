...
module type Bt_SIG =
  sig
    ...
    (*val to_rb : 'a t_234tree -> 'a t_rbtree*)
...
module BtSum : Bt_SIG =
  struct
        ...
        | Rooted_1(x, l, r) ->
          if a < x then 
            ...
            | Rooted_3(x1, x2, x3, l2, m2, n2, r2) -> 
                if a < x2 then Rooted_2(x2, x, bt_add(a,Rooted_1(x1,l2,m2)), Rooted_1(x3,n2,r2), r)
                else Rooted_2(x2, x, Rooted_1(x1,l2,m2), bt_add(a,Rooted_1(x3,n2,r2)), r)
    ...
    (* Question 6 *)
    (*let rec to_rb(tree : 'a t_234tree) : 'a t_rbtree = 
      match tree with
      | Empty -> rbt_empty()
      | Rooted_1(v, l, r) -> node(black(),v,to_rb(l),to_rb(r))
      | Rooted_2(v,v2, l,m, r) -> node(black(),v2,node(red(),v,to_rb(l),to_rb(m)),to_rb(r))
      | Rooted_3(v,v2,v3, l,m,n, r) -> node(black(),v2,node(red(),v,to_rb(l),to_rb(m)),node(red(),v3,to_rb(n),to_rb(r)))
     ;;*)
...
module type RBT_SIG =
  sig
    ...
    val to_234 : 'a t_rbtree -> 'a t_234tree
...
B pour Black et R pour Red
  ...
  a    b             b    c       b   c               c    d 
    ...
    *)
    ...
    let rec to_234(tree : 'a t_rbtree) : 'a t_234tree =
      
      match tree with
      | Leaf -> bt_empty()
      | Node(c, v, l, r) ->
         if rbt_isempty(l) && rbt_isempty(r) then rooted_1(v,bt_empty(),bt_empty())
         else
           if rbt_isempty(l) then
             match r with
             | Node(c2,v2,l2,r2) ->
                if c = Red then rooted_2(v,v2,bt_empty(),to_234(l2),to_234(r2))
                else rooted_1(v,bt_empty(),to_234(r))
             | _ ->failwith "impossible d'arrivé la"
           else
             if rbt_isempty(r) then
               match l with
               | Node(c2,v2,l2,r2) ->
                  if c = Red then rooted_2(v2,v,to_234(l2),to_234(r2),bt_empty())
                  else rooted_1(v,to_234(l),bt_empty())
               | _ ->failwith "impossible d'arrivé la"
             else
               match l with
               | Node(cl,vl,ll,rl) ->
                  (match r with
                     | Node(cr,vr,lr,rr) ->
                        if cl = Red then
                          if cr = Red then
                            rooted_3(vl,v,vr,to_234(ll),to_234(rl),to_234(lr),to_234(rr))
                          else
                            rooted_2(vl,v,to_234(ll),to_234(rl),to_234(r))
                        else
                          if cr = Red then
                            rooted_2(v,vr,to_234(l),to_234(lr),to_234(rr))
                          else
                            rooted_1(v,to_234(l),to_234(r))
                     | _ ->failwith "impossible d'arrivé la")
               | _ ->failwith "impossible d'arrivé la"
  end
;;
...
rbt_search(40, !test2);; (*false*)
    ...
    (* test Question 6*)
to_rb(bt_add(6,bt_add(11,bt_add(20,bt_add(42,rooted_2(5,15,bt_empty(),bt_empty(),bt_empty()))))));; 
